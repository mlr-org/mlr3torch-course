...
$$a_{t+1} = a_t - \eta \frac{\partial R}{\partial a_t}$$
$$b_{t+1} = b_t - \eta \frac{\partial R}{\partial b_t}$$
...
Note that if we repeatedly call `loss$backward()`, the gradients in `a` and `b` would accumulate, so we set them to `0` before performing the update.
The return value of the update will be the parameter values and the loss so we can plot them later.
