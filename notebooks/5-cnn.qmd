...
![](../assets/padding_strides.png)

[Source](https://www.geeksforgeeks.org/cnn-introduction-to-padding/)

For the input and output channels, we have the following:

![](../assets/input_output_channels.svg)

[Source](https://classic.d2l.ai/chapter_convolutional-neural-networks/channels.html)
...
Below, we demonstrate it in action. We start by applying a convolutional layer to an MNIST image to obtain feature maps and then apply a 2×2 max pooling layer (with stride 2) on the convolution output.
The two results are visualized side-by-side.
...
While we have no covered individual components of CNNs, the question of how to configure and compose them is a challenging task, but essential for building efficient neural networks.
...
The ResNet-18 model is a well-known pre-trained model that was pretrained on ImageNet.
It's (complicated) architecture is shown below:
...
In the case of MNIST, a predefined dataloader is available in the `torchvision` package.
...
We can now compare the pretrained and untrained resnet on the training task and evaluate it on the validation data.
The results are shown below:

```{r, echo = FALSE, eval = torch::cuda_is_available()}
library(data.table)
