...
They define how a network *learns* from data.
...
- $\nabla_{\theta_t} \hat{\mathcal{R}}$ is the gradient of the empirical risk function $\hat{\mathcal{R}}$ with respect to parameter $\theta$, evaluated at $\theta_t$.
...
<details>
<summary>Click for answer</summary>
In machine learning, we assume that the data is drawn from a distribution $\mathbb{E}$.
...
$$g_t = \beta_2 g_{t-1} + (1-\beta_2)(\nabla_\theta \hat{\mathcal{R}}^B_t)^2$$
$$\hat{\eta}_t = \eta \frac{1}{\sqrt{g_t} + \epsilon}$$
...
1. The `$param_groups` which contains the hyperparameters for each parameter group.
...
Besides the explicit regularization effects of weight decay and the implicit regularization effects of mini-batch gradient descent, there are also other techniques that improve generalization of deep neural networks.
Here, we focus on dropout and batch normalization, which are both embedded in neural network architectures.
...
To demonstrate these methods, we apply them to a simple spam classification task.
...
While the intervals are too wide to be able to draw any final conclusions, dropout here seems to have a positive effect on the generalization performance of the model, whereas batch normalization does not (which is not too surprising as it is usually used with convolutional layers).
