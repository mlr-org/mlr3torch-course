...
```{r, message = FALSE}
library(torch)
data <- mlbench::mlbench.friedman3(n = 1000, sd = 0.1)
...
The purpose of this exercise is *not* to derive observations that hold in general.

::: {.content-visible when-meta=solutions}
**Solution**

There is not really a 'solution' to this exercise.
We will go through some of the configurations and try to explain what is happening.
...
Too large learning rates lead to unstable updates and divergence.
It is not exactly clear why the small learning rate is so unstable, possibly it gets stuck in a bad region of the loss landscape.
...
Note that this comparison is not entirely fair as the number of epochs is fixed.
We would rather be interested in the performance of different batch sizes for a fixed amount of *time* and not *epochs*.
...
We can observe that too large values for the momentum parameter lead to oscillations, possibly because the local information is not used enough.
Too small values lead to worse performance.
...
In this exercise, you will build a (simplified) gradient descent optimizer with momentum.
...
It will receive as arguments, the current values `x` and `y`, the momentum values `x_momentum` and `y_momentum` (all scalar tensors) as well as the learning rate `lr` and the momentum parameter `beta`.
...
To test your optimizer, you can use the code below.
Note that you might have to play around with the number of steps and the learning rate to get a good result.

<details>
<summary>Code to test your optimizer</summary>
...
Does it make sense to also use weight decay here?
