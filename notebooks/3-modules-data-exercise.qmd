...
* a 2D tensor (shape `(n, 1)`) `x` of `n` values drawn from a uniform distribution between `min` and `max`
* a 2D tensor (shape `(n, 1)`) `y` that is defined as $\sin(x) + \epsilon$ where $\epsilon$ is drawn from a normal distribution with mean 0 and standard deviation 0.1
